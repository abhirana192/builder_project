...
export default function Transport() {
  ...
  const [scheduleType, setScheduleType] = useState<'airport' | 'activity' | 'custom'>('airport');
  ...
  const [selectedPassengers, setSelectedPassengers] = useState<Array<{id: number, type: 'group' | 'member', data: Group | GroupMember}>>([]);
  const [passengerSearchTerm, setPassengerSearchTerm] = useState('');
  const [selectedActivity, setSelectedActivity] = useState('');
  const [pickupLocation, setPickupLocation] = useState('');
  const [dropoffLocation, setDropoffLocation] = useState('');
  const [isDetailDialogOpen, setIsDetailDialogOpen] = useState(false);
  const [selectedScheduleForDetail, setSelectedScheduleForDetail] = useState<TransportSchedule | null>(null);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [scheduleToDelete, setScheduleToDelete] = useState<TransportSchedule | null>(null);
      ...
      const groupsWithMembers = await Promise.all(
        groupsData.map(async (group: any) => {
          try {
            ...
            console.warn(`Failed to fetch detailed data for group ${group.id}:`, error);
            return group;
          }
        })
      );
  ...
  const formatTimeAMPM = (timeString: string, dateString?: string) => {
    ...
    try {
      ...
      const baseDate = dateString ? dateString.split('T')[0] : new Date().toISOString().split('T')[0];
  ...
  const getFlightDetails = (member: GroupMember | Group, transferType: 'pickup' | 'dropoff') => {
    const isGroup = 'group_name' in member;

    if (transferType === 'pickup') {
      const flightNumber = isGroup ? member.arrival_flight_number : (member as GroupMember).arrival_flight_number;
      const flightTime = isGroup ? member.arrival_flight_time : (member as GroupMember).arrival_flight_time;
      ...
      const flightNumber = isGroup ? member.departure_flight_number : (member as GroupMember).departure_flight_number;
      const flightTime = isGroup ? member.departure_flight_time : (member as GroupMember).departure_flight_time;
  ...
  const isPassengerAlreadyAssigned = (passengerId: number, passengerType: 'group' | 'member', currentTransportType?: string) => {
    // Check if passenger is already assigned to the same transport type
    for (const schedule of schedules) {
      if (schedule.groups_data) {
        try {
          const groupsData = typeof schedule.groups_data === 'string'
            ? JSON.parse(schedule.groups_data)
            : schedule.groups_data;

          if (Array.isArray(groupsData)) {
            const isAssigned = groupsData.some((assignedPassenger: any) =>
              assignedPassenger.type === passengerType && assignedPassenger.id === passengerId
            );

            // Only consider it assigned if it's the same transport type (or if no type specified, check all)
            if (isAssigned && (!currentTransportType || schedule.transport_type === currentTransportType)) {
              return {
                assigned: true,
          ...
          console.error('Error parsing groups_data for schedule:', schedule.id, error);
        }
      }
  ...
  const getAssignmentStatus = (passenger: Group | GroupMember, type: 'group' | 'member') => {
  ...
  const calculateTransportTime = (flightTime: string, flightDate: string, transferType: 'pickup' | 'dropoff') => {
    // Validate parameters first
    if (typeof flightTime !== 'string' || typeof flightDate !== 'string' || typeof transferType !== 'string') {
      console.error('Invalid parameter types:', { flightTime: typeof flightTime, flightDate: typeof flightDate, transferType: typeof transferType });
      return '';
    }
    ...
    if (transferType.includes(':') || flightTime === 'pickup' || flightTime === 'dropoff') {
      console.error('Parameters appear to be in wrong order:', { flightTime, flightDate, transferType });
      return '';
    }

    try {
      // Debug logging
      console.log('Calculating transport time with:', { flightTime, flightDate, transferType });
      ...
      if (!timeStr.includes(':')) {
        console.warn('Flight time does not include colon, defaulting to 12:00:', timeStr);
        timeStr = '12:00';
      }
  ...
  const handlePassengerSelection = (checked: boolean, type: 'group' | 'member', data: Group | GroupMember) => {
    if (checked) {
      ...
      if (selectedPassengers.length === 0) {
        ...
        console.log('Extracted flight data:', { flightTime, flightDate, flightNum, selectedTransferType });

        if (flightTime && flightDate) {
          // Ensure date is in YYYY-MM-DD format
          const dateStr = flightDate.includes('T') ? flightDate.split('T')[0] : flightDate.split(' ')[0];
          ...
          console.log('Constructing flight datetime:', { flightDate, flightTime, dateStr, timeStr, fullFlightDateTime });
          setOriginalFlightTime(fullFlightDateTime);
          setFlightNumber(flightNum);
        } else {
          console.warn('Missing flight data:', { flightTime, flightDate, type, data });
        }
      }
    } else {
      setSelectedPassengers(prev => prev.filter(p => !(p.id === data.id && p.type === type)));
  ...
  const handleTransferTypeChange = (transferType: 'pickup' | 'dropoff') => {
    ...
    if (selectedPassengers.length > 0) {
      ...
      if (flightTime && flightDate) {
        ...
        const fullFlightDateTime = `${dateStr}T${timeStr}:00`;
        console.log('Constructing flight datetime (transfer type change):', { flightDate, flightTime, dateStr, timeStr, fullFlightDateTime });
        setOriginalFlightTime(fullFlightDateTime);
        setFlightNumber(flightNum);
      }
    }
  ...
  const handleTransferTypeSelection = (transferType: 'pickup' | 'dropoff') => {
    ...
    if (selectedCustomer) {
      ...
      if (flightTime && flightDate) {
        ...
        const fullFlightDateTime = `${dateStr}T${timeStr}:00`;
        console.log('Constructing flight datetime (transfer type selection):', { flightDate, flightTime, dateStr, timeStr, fullFlightDateTime });
        setOriginalFlightTime(fullFlightDateTime);
        setFlightNumber(flightNum);
        setCurrentStep(3);
      } else {
        console.warn('No flight details found for selected customer and transfer type');
  ...
  const handleScheduleTransport = async () => {
    if (!pickupLocation || !dropoffLocation || !selectedVehicle || !selectedDriver) {
      return;
    }
    ...
    try {
      ...
      if (!flightTimeToUse && selectedPassengers.length > 0 && (selectedTransferType === 'pickup' || selectedTransferType === 'dropoff')) {
        ...
        if (flightTime && flightDate) {
          ...
          console.log('��� Reconstructed flight time from passenger data:', flightTimeToUse);
        }
      }

      if (flightTimeToUse && (selectedTransferType === 'pickup' || selectedTransferType === 'dropoff')) {
        ...
        console.warn('⚠️ No flight time available for calculation. Using current time.', {
          flightTimeToUse,
  ...
  const handleStatusChange = async (scheduleId: number, newStatus: string) => {
    try {
      const response = await fetch(`/api/transport/schedules/${scheduleId}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        if (selectedScheduleForDetail && selectedScheduleForDetail.id === scheduleId) {
  ...
  const confirmDeleteTransport = async () => {
    ...
    try {
      const response = await fetch(`/api/transport/schedules/${scheduleToDelete.id}`, {
        method: 'DELETE',
      });
  ...
  const handleDropoffLocationUpdate = async () => {
    ...
    try {
      const response = await fetch(`/api/transport/schedules/${selectedScheduleForDetail.id}/dropoff-location`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ dropoff_location: editingDropoffLocation.trim() }),
      });
  ...
  const handlePickupLocationUpdate = async () => {
    ...
    try {
      const response = await fetch(`/api/transport/schedules/${selectedScheduleForDetail.id}/pickup-location`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ pickup_location: editingPickupLocation.trim() }),
      });
    ...
    <div className="container mx-auto px-4 py-8 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Select value={selectedTransferType} onValueChange={(value: 'pickup' | 'dropoff' | 'custom') => {
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={selectedTransferType === 'pickup' ? 'Yellowknife Airport' : 'Enter pickup address or location'}
                        className="mt-1"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={selectedTransferType === 'dropoff' ? 'Yellowknife Airport' : 'Enter dropoff address or destination'}
                        className="mt-1"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Select value={selectedTransferType} onValueChange={handleTransferTypeChange}>
                          <SelectTrigger>
                          ...
                          {originalFlightTime && (
                            <div className="mt-2 p-3 bg-blue-50 rounded-md border">
                              <div className="text-sm text-blue-700">
                                <div className="font-medium mb-2">Original Flight Details:</div>
                                <div className="flex items-center justify-between bg-white p-2 rounded border">
                                  <div>
                                    <div className="font-semibold text-blue-800">Flight {flightNumber}</div>
                                    <div className="text-blue-600">{new Date(originalFlightTime).toLocaleString()}</div>
                                  </div>
                                  <div className="text-right">
                                    <div className="text-xs text-blue-600 font-medium">
                                      Original {selectedTransferType === 'pickup' ? 'arrival' : 'departure'} time
                                    </div>
                                    <div className="text-xs text-blue-500">
                                      Transport scheduled at original flight time: {new Date(originalFlightTime).toLocaleDateString()} {new Date(originalFlightTime).toLocaleTimeString()}
                                    </div>
                                  </div>
                                </div>
                              </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <Label className="text-base font-semibold">Select Passengers</Label>
                      ...
                      {selectedPassengers.length > 0 && (
                        <div className="mb-4 p-3 bg-blue-50 rounded-lg border">
                          <div className="text-sm font-medium text-blue-800 mb-2">Selected Passengers:</div>
                          <div className="flex flex-wrap gap-2">
                            {selectedPassengers.map((passenger, index) => (
                              <div key={`${passenger.type}-${passenger.id}`} className="flex items-center space-x-1 bg-white px-2 py-1 rounded border">
                                ...
                                <button
                                  onClick={() => handlePassengerSelection(false, passenger.type, passenger.data)}
                      ...
                      <div className="max-h-72 overflow-y-auto border rounded-lg">
                        {groups
                          ...
                          .filter(group => {
                            ...
                            // Search group name
                            if (group.group_name?.toLowerCase().includes(searchLower)) return true;
                            ...
                            <div key={group.id} className="border-b last:border-b-0">
                              {group.traveling_together ? (
                                ...
                                <div className="p-3 hover:bg-gray-50 transition-colors">
                                  <div className="flex items-center space-x-3">
                                    <input
                                      type="checkbox"
                                      id={`group-${group.id}`}
                                      checked={isPassengerSelected('group', group.id)}
                                      onChange={(e) => handlePassengerSelection(e.target.checked, 'group', group)}
                                      disabled={getAssignmentStatus(group, 'group').assigned}
                                    ...
                                    <div className="flex-1">
                                      <div className="flex items-center space-x-2">
                                        ...
                                        {getAssignmentStatus(group, 'group').assigned && (
                                      ...
                                      <div className="text-sm text-gray-600 mt-1">
                                        {selectedTransferType === 'pickup' && group.arrival_flight_number && (
                                          <span className="text-blue-600">✈️ Arrival: {group.arrival_flight_number} at {group.arrival_flight_time}</span>
                                        )}
                                        {selectedTransferType === 'dropoff' && group.departure_flight_number && (
                                          <span className="text-orange-600">�� Departure: {group.departure_flight_number} at {group.departure_flight_time}</span>
                                        )}
                                      </div>
                                    </div>
                                  </div>
                                ...
                                <div className="p-3">
                                  ...
                                  {group.traveling_together && selectedTransferType !== 'custom' && (
                                        ...
                                        <div className="flex-1">
                                          <div className="flex items-center space-x-2">
                                            <span className="font-medium text-blue-800">
                                              Select entire group ({group.total_members} passengers)
                                            </span>
                                            {getAssignmentStatus(group, 'group').assigned && (
                                              <Badge variant="destructive" className="text-xs">
                                                Already Assigned
                                          ...
                                          <div className="text-sm text-blue-600 mt-1">
                                            {selectedTransferType === 'pickup' && group.arrival_flight_number && (
                                              <span>✈️ Arrival: {group.arrival_flight_number} at {group.arrival_flight_time}</span>
                                            )}
                                            {selectedTransferType === 'dropoff' && group.departure_flight_number && (
                                              <span>🛫 Departure: {group.departure_flight_number} at {group.departure_flight_time}</span>
                                            )}
                                          </div>
                                        </div>
                                      </div>
                                  ...
                                  {group.members && group.members.length > 0 && (
                                    <div className="ml-4 space-y-2">
                                      ...
                                      {group.members
                                        .filter(member => {
                                          if (!passengerSearchTerm) return true;
                                          return `${member.first_name} ${member.last_name}`.toLowerCase().includes(passengerSearchTerm.toLowerCase());
                                        })
                                        .map(member => {
                                          const assignmentStatus = getAssignmentStatus(member, 'member');
                                          const isAlreadyAssigned = assignmentStatus.assigned;

                                          return (
                                          <div key={member.id} className={`flex items-center space-x-3 p-3 rounded-md transition-colors border ${
                                            isAlreadyAssigned
                                            ...
                                            <input
                                              type="checkbox"
                                              id={`member-${member.id}`}
                                              checked={isPassengerSelected('member', member.id)}
                                              onChange={(e) => handlePassengerSelection(e.target.checked, 'member', member)}
                                            ...
                                            <div className="flex-1">
                                              <div className="flex items-center justify-between">
                                                <div className="flex items-center space-x-2">
                                                  <span className={`font-semibold ${isAlreadyAssigned ? 'text-gray-500' : 'text-gray-900'}`}>
                                                    {member.first_name} {member.last_name}
                                                  </span>
                                                  {member.is_leader && (
                                                    <Badge variant="outline" className="text-xs bg-blue-100 text-blue-700 border-blue-300">Leader</Badge>
                                                  )}
                                                  {isAlreadyAssigned && (
                                                    <Badge variant="destructive" className="text-xs">
                                                      Already Assigned
                                              ...
                                              {isAlreadyAssigned && (
                                                <div className="mt-1 text-xs text-red-600 bg-red-100 px-2 py-1 rounded">
                                                  🚗 Assigned to {assignmentStatus.vehicleNumber} - {assignmentStatus.transportType?.replace('_', ' ')}
                                                  at {new Date(assignmentStatus.pickupTime).toLocaleString()}
                                                </div>
                                              )}
                                              ...
                                              {selectedTransferType !== 'custom' && (
                                                <div className="mt-2 space-y-1">
                                                  {(() => {
                                                    const arrivalDetails = getFlightDetails(member, 'pickup');
                                                    const departureDetails = getFlightDetails(member, 'dropoff');

                                                    return (
                                                      <>
                                                        {arrivalDetails.flightNumber && (
                                                          <div className={`text-xs flex items-center space-x-2 ${
                                                            selectedTransferType === 'pickup' ? 'font-medium text-blue-700 bg-blue-100 px-2 py-1 rounded' : 'text-gray-600'
                                                          }`}>
                                                            <span>✈️ Arrival:</span>
                                                            <span className="font-mono font-semibold">{arrivalDetails.flightNumber}</span>
                                                            <span>{arrivalDetails.flightTime}</span>
                                                          </div>
                                                        )}
                                                        {departureDetails.flightNumber && (
                                                          <div className={`text-xs flex items-center space-x-2 ${
                                                            selectedTransferType === 'dropoff' ? 'font-medium text-orange-700 bg-orange-100 px-2 py-1 rounded' : 'text-gray-600'
                                                          }`}>
                                                            <span>🛫 Departure:</span>
                                                            <span className="font-mono font-semibold">{departureDetails.flightNumber}</span>
                                                            <span>{departureDetails.flightTime}</span>
                                                          </div>
                                                        )}
                                                        {!arrivalDetails.flightNumber && !departureDetails.flightNumber && (
                        ...
                        {/* No results message */}
                        {passengerSearchTerm && groups.filter(g => {
                          const searchLower = passengerSearchTerm.toLowerCase();
                          ...
                          <div className="p-8 text-center text-gray-500">
                            ...
                            <p>No groups or passengers found matching "{passengerSearchTerm}"</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {scheduleType === 'activity' && (
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <Label className="text-base font-semibold">Select Participants</Label>
                        {selectedPassengers.length > 0 && (
                      ...
                      {selectedPassengers.length > 0 && (
                        <div className="mb-4 p-3 bg-green-50 rounded-lg border">
                          <div className="text-sm font-medium text-green-800 mb-2">Selected Participants:</div>
                          <div className="flex flex-wrap gap-2">
                                  ...
                                  {group.members && group.members.length > 0 && (
                                    <div className="ml-4 space-y-2">
                                      {group.members
                                        ...
                                        .map(member => (
                                          <div key={member.id} className="flex items-center space-x-3 p-3 bg-purple-50 rounded-md hover:bg-purple-100 transition-colors border border-purple-200">
                                            <input
                                              type="checkbox"
                                                  ...
                                                  return (
                                                    <>
                                                      ...
                                                      <div className="text-xs text-purple-600 font-medium">
                                                        Available for activity transport
                                                      </div>
                                                    </>
                                                  );
                                                })()
                    ...
                    <div>
                      ...
                      {selectedPassengers.length > 0 && (
                        <div className="mb-4 p-3 bg-purple-50 rounded-lg border">
                          <div className="text-sm font-medium text-purple-800 mb-2">Selected Passengers:</div>
                          <div className="flex flex-wrap gap-2">
                            {selectedPassengers.map((passenger) => (
                              <div key={`${passenger.type}-${passenger.id}`} className="flex items-center space-x-1 bg-white px-2 py-1 rounded border">
                                <span className="text-xs">
                                  ...
                                  {group.members && group.members.length > 0 && (
                                    <div className="ml-4 space-y-2">
                                      {group.members
                                        ...
                                        .map(member => (
                                          <div key={member.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-md hover:bg-gray-100 transition-colors border border-gray-200">
                                            <input
                                              type="checkbox"
                  ...
                  {selectedVehicle && selectedPassengers.length > 0 && (
                    <div className="p-3 rounded-lg border">
                      {(() => {
                        ...
                        const totalPassengers = selectedPassengers.reduce((total, passenger) => {
                          if (passenger.type === 'group') {
                            return total + (passenger.data as Group).total_members;
                          } else {
                            return total + 1;
                        ...
                        const isOverCapacity = vehicle && totalPassengers > vehicle.capacity;
                        ...
                        return (
                          <div className={`flex items-center space-x-2 text-sm ${
                            ...
                            <span>
                              {totalPassengers} passengers / {vehicle?.capacity} capacity
                ...
                <div className="flex flex-col space-y-3">
                  ...
                  <div className="flex justify-end space-x-3">
                    ...
                    <Button
                      className="px-6"
                      disabled={!pickupLocation || !dropoffLocation || !selectedVehicle || !selectedDriver || isSubmitting}
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm font-semibold">{selectedScheduleForDetail.vehicle_number} - {selectedScheduleForDetail.vehicle_type}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-600">Driver</Label>
                      <p className="text-sm font-semibold">{selectedScheduleForDetail.driver_name}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div>
                      ...
                      <p className="text-sm font-semibold">{formatTransportType(selectedScheduleForDetail.transport_type || 'custom')}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-600">Current Status</Label>
                      <Badge variant={getStatusColor(selectedScheduleForDetail.status)} className="w-fit">
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg border">
                    <Label className="text-sm font-medium text-gray-700 mb-3 block">Change Status</Label>
                    <div className="grid grid-cols-3 gap-3">
                      <button
                        onClick={() => handleStatusChange(selectedScheduleForDetail.id, 'scheduled')}
                      ...
                      <button
                        onClick={() => handleStatusChange(selectedScheduleForDetail.id, 'in_transit')}
                        className={`px-3 py-2 text-xs font-medium rounded-md border transition-all duration-200 ${
                      ...
                      <button
                        onClick={() => handleStatusChange(selectedScheduleForDetail.id, 'completed')}
                        className={`px-3 py-2 text-xs font-medium rounded-md border transition-all duration-200 ${
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      {(selectedScheduleForDetail.transport_type === 'airport_dropoff') ? (
                        isEditingPickup ? (
                          <div className="flex items-center space-x-2 mt-1">
                            <Input
                              value={editingPickupLocation}
                              onChange={(e) => setEditingPickupLocation(e.target.value)}
                              placeholder="Enter pickup location"
                              className="text-sm"
                            />
                            <Button size="sm" onClick={handlePickupLocationUpdate} disabled={!editingPickupLocation.trim()}>
                              Save
                            </Button>
                            <Button size="sm" variant="outline" onClick={cancelEditingPickup}>
                              Cancel
                            </Button>
                          </div>
                        ) : (
                          <div className="flex items-center justify-between mt-1">
                            <p className="text-sm">{selectedScheduleForDetail.pickup_location}</p>
                            <Button size="sm" variant="ghost" onClick={startEditingPickup} className="text-blue-600 hover:text-blue-800">
                              Edit
                            </Button>
                          </div>
                        )
                    ...
                    <div>
                      ...
                      {(selectedScheduleForDetail.transport_type === 'airport_pickup' || selectedScheduleForDetail.transport_type === 'custom') ? (
                        isEditingDropoff ? (
                          <div className="flex items-center space-x-2 mt-1">
                            <Input
                              value={editingDropoffLocation}
                              onChange={(e) => setEditingDropoffLocation(e.target.value)}
                              placeholder="Enter dropoff location"
                              className="text-sm"
                            />
                            <Button size="sm" onClick={handleDropoffLocationUpdate} disabled={!editingDropoffLocation.trim()}>
                              Save
                            </Button>
                            <Button size="sm" variant="outline" onClick={cancelEditingDropoff}>
                              Cancel
                            </Button>
                          </div>
                        ) : (
                          <div className="flex items-center justify-between mt-1">
                            <p className="text-sm">{selectedScheduleForDetail.dropoff_location}</p>
                            <Button size="sm" variant="ghost" onClick={startEditingDropoff} className="text-blue-600 hover:text-blue-800">
                              Edit
                            </Button>
                          </div>
                        )
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="text-sm font-medium text-gray-600">{getTimeLabel(selectedScheduleForDetail.transport_type)} Time</Label>
                      <p className="text-sm">{formatDateTime(selectedScheduleForDetail.pickup_time)}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-600">Passenger Count</Label>
                      <p className="text-sm">{selectedScheduleForDetail.passenger_count || 0} passengers</p>
                    </div>
                  </div>
                  ...
                  {selectedScheduleForDetail.activity_name && (
                    <div>
                      ...
                      <p className="text-sm">{selectedScheduleForDetail.activity_name}</p>
                    </div>
                  )}
                  ...
                  {selectedScheduleForDetail.distance_km && (
                    <div>
                      ...
                      <p className="text-sm">{selectedScheduleForDetail.distance_km} km</p>
                    </div>
                  )}

                  {selectedScheduleForDetail.groups && selectedScheduleForDetail.groups.length > 0 && (
                    <div>
                      <Label className="text-sm font-medium text-gray-600">Passengers</Label>
                      <div className="mt-2 space-y-2">
                        {getDetailedPassengerInfo(selectedScheduleForDetail).map((passenger, index) => (
                          <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                            <div className="flex items-center space-x-3">
                              <div className={`w-2 h-2 rounded-full ${passenger.type === 'group' ? 'bg-blue-500' : 'bg-green-500'}`}></div>
                              <div>
                                <div className="font-medium text-gray-900">{passenger.name}</div>
                                <div className="text-sm text-gray-600">
                                  {passenger.type === 'group' ? 'Group' : `Member of ${passenger.groupName}`}
                                </div>
                              </div>
                            </div>
                            <div className="text-right">
                              <div className="text-sm font-medium text-gray-700">
                                {passenger.memberCount} {passenger.memberCount === 1 ? 'passenger' : 'passengers'}
                              </div>
                              <div className="text-xs text-gray-500">
                                {passenger.type === 'group' ? 'Traveling together' : 'Individual'}
                              </div>
                            </div>
                          </div>
                        ))}
                        <div className="mt-3 p-2 bg-blue-50 rounded border border-blue-200">
                          <div className="text-sm text-blue-700 font-medium">
                            Total: {getDetailedPassengerInfo(selectedScheduleForDetail).reduce((sum, p) => sum + p.memberCount, 0)} passengers
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
            <DialogContent className="sm:max-w-[400px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  This action cannot be undone. Are you sure you want to delete this transport schedule?
                </DialogDescription>
              </DialogHeader>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="font-medium text-red-800">
                          {scheduleToDelete.vehicle_number} - {scheduleToDelete.vehicle_type}
                        </span>
                      </div>
                      <div className="text-sm text-red-700">
                        ...
                        <div className="flex items-center space-x-1 mt-1">
                          ...
                          <span>{getTimeLabel(scheduleToDelete.transport_type)} Time: {formatDateTime(scheduleToDelete.pickup_time)}</span>
                        </div>
                        <div className="flex items-center space-x-1 mt-1">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-2xl font-bold">{schedules.filter(s => s.status === 'in_transit').length}</p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">
                      {schedules.filter(s => s.transport_type === 'airport_pickup').length}
                    </p>
                    <p className="text-sm text-muted-foreground">Airport Pickups</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">
                      {schedules.filter(s => s.transport_type === 'airport_pickup').reduce((sum, s) => sum + (s.passenger_count || 0), 0)}
                    </p>
                    <p className="text-sm text-muted-foreground">Pickup Passengers</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {schedules.length === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-semibold text-foreground mb-2">No transport schedules</h3>
                <p className="text-muted-foreground">No scheduled transports found for today</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
        ...
        <TabsContent value="airport" className="space-y-4">
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">
                      {schedules.filter(s => s.transport_type === 'airport_dropoff').length}
                    </p>
                    <p className="text-sm text-muted-foreground">Airport Dropoffs</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">
                      {schedules.filter(s => s.transport_type === 'airport_dropoff').reduce((sum, s) => sum + (s.passenger_count || 0), 0)}
                    </p>
                    <p className="text-sm text-muted-foreground">Dropoff Passengers</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {schedules.filter(s => s.transport_type?.includes('airport')).length === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                <Plane className="mx-auto h-12 w-12 text-muted-foreground mb-4" />
                <h3 className="text-lg font-semibold text-foreground mb-2">No airport transfers scheduled</h3>
                <p className="text-muted-foreground">Schedule airport pickups and dropoffs for your groups</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
        ...
        <TabsContent value="activities" className="space-y-4">
              ...
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Activity Transports</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">
                      {schedules.filter(s => s.transport_type === 'activity').reduce((sum, s) => sum + (s.passenger_count || 0), 0)}
                    </p>
                    <p className="text-sm text-muted-foreground">Activity Participants</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">
                      {new Set(schedules.filter(s => s.transport_type === 'activity').map(s => s.activity_name)).size}
                    </p>
                    <p className="text-sm text-muted-foreground">Different Activities</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid gap-4">
            {schedules.filter(s => s.transport_type === 'activity').map((schedule) => {
              const VehicleIcon = getVehicleIcon(schedule.vehicle_type);

              return (
                <Card key={schedule.id} className="hover:shadow-lg transition-shadow border-l-4 border-l-green-500">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-2">
                            <h3 className="text-lg font-semibold text-foreground">{schedule.activity_name}</h3>
                            <Badge variant="secondary">{schedule.vehicle_number}</Badge>
                            <Badge variant={getStatusColor(schedule.status)}>{schedule.status}</Badge>
                          </div>
                            ...
                            <div className="mb-3 p-3 bg-green-50 rounded-lg">
                              <div className="flex items-center space-x-2 mb-1">
                                ...
                                <span className="text-sm font-medium text-green-800">
                                  Participating Groups: {formatPassengerNames(schedule.groups)}
                                </span>
                              </div>
                              <p className="text-sm text-green-600">
                                Total participants: {schedule.passenger_count || 0}
                              </p>
                            </div>
                          )}
                        ...
                        <div className="text-right">
                          <div className="text-2xl font-bold text-green-600">{schedule.passenger_count || 0}</div>
          ...
          {schedules.filter(s => s.transport_type === 'activity').length === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                <Mountain className="mx-auto h-12 w-12 text-muted-foreground mb-4" />
                <h3 className="text-lg font-semibold text-foreground mb-2">No activity transports scheduled</h3>
                <p className="text-muted-foreground">Schedule transportation for ice fishing, city tours, northern lights, and other activities</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
        ...
        <TabsContent value="vehicles" className="space-y-4">
          ...
          <div className="grid gap-4">
            {vehicles.map((vehicle) => {
              ...
              const maintenanceDue = new Date(vehicle.next_maintenance) <= new Date();
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-1">
                            <h3 className="text-lg font-semibold text-foreground">{vehicle.vehicle_number}</h3>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm text-muted-foreground">
                            ...
                            <div>Last Service: {formatDate(vehicle.last_maintenance)}</div>
                            <div>Next Service: {formatDate(vehicle.next_maintenance)}</div>
                          </div>
                          {vehicle.notes && (
                            <p className="text-sm text-muted-foreground mt-2 italic">{vehicle.notes}</p>
                          )}
                        </div>
                        <div className="flex items-center space-x-2">
                          <StatusIcon className="h-6 w-6 text-muted-foreground" />
                          <Select onValueChange={(value) => updateVehicleStatus(vehicle.id, value)}>
          ...
          {vehicles.length === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-muted-foreground">Add vehicles to your fleet to get started</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
