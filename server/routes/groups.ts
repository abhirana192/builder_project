...
const autoScheduleTransportOLD = async (groupId: number, group: any, members: any[]) => {
  try {
    console.log('\n\n==============================================');
    ...
    console.log('- Traveling Together:', JSON.stringify(group.traveling_together));
    console.log('- ARRIVAL DATE:', JSON.stringify(group.arrival_date));
    console.log('- ARRIVAL FLIGHT TIME:', JSON.stringify(group.arrival_flight_time));
    console.log('- DEPARTURE DATE:', JSON.stringify(group.departure_date));
    console.log('- DEPARTURE FLIGHT TIME:', JSON.stringify(group.departure_flight_time));
    ...
    const vehicles = allVehicles.filter((vehicle: any) => vehicle.status === 'available');
    const drivers = queries.getAllStaff().all().filter((staff: any) => staff.role === 'driver' && staff.is_active);
    ...
    if (group.traveling_together) {
      ...
      console.log('👤 Group traveling separately - creating individual transports');
      await createTransportForSeparateMembers(group, members, vehicles, drivers);
    }
...
const createTransportForGroup = async (group: any, members: any[], vehicles: any[], drivers: any[]) => {
  console.log('Creating transport for group traveling together:', group.group_name);
  ...
  if (group.arrival_date && group.arrival_flight_time) {
    ...
    console.log('Raw arrival_flight_time:', JSON.stringify(group.arrival_flight_time));
    ...
    if (group.arrival_date.includes('T')) {
      ...
      const timeWithSeconds = group.arrival_flight_time.includes(':') && group.arrival_flight_time.split(':').length === 2
    ...
    console.log('Constructed flight datetime string:', JSON.stringify(flightDateTime));
    ...
    console.log('ISO string to store:', JSON.stringify(pickupTime.toISOString()));
    ...
    const pickupSchedule = {
      ...
      estimated_dropoff_time: new Date(pickupTime.getTime() + 60 * 60 * 1000).toISOString(),
  ...
  if (group.departure_date && group.departure_flight_time) {
    console.log('📅 Creating dropoff transport for departure:', group.departure_date, group.departure_flight_time);
    // Ensure time has seconds if not present
    const departureTimeFormatted = group.departure_flight_time.includes(':') && group.departure_flight_time.split(':').length === 2 ? `${group.departure_flight_time}:00` : group.departure_flight_time;
    ...
    console.log('  - Departure date parts:', yearDep, monthDep, dayDep, hourDep, minuteDep, secondDep);

    console.log('���� DEBUG - Departure flight time from group data:', group.departure_flight_time);
    console.log('🔍 DEBUG - Constructed pickup time string:', departureTimeString);
...
const createTransportForSeparateMembers = async (group: any, members: any[], vehicles: any[], drivers: any[]) => {
  console.log('Creating transport for members traveling separately:', group.group_name);
  ...
  for (const member of members) {
    ...
    if (freshData) {
      ...
      console.log(`🔄 Fetched fresh flight data for member ${member.id}:`, freshData);
    }
  }
  ...
  for (const [arrivalKey, arrivalMembers] of arrivalGroups) {
    ...
    console.log('🔍 DEBUG - Member arrival transport - Original time:', time, 'Formatted:', timeFormatted);
    ...
    await createTransportSchedule({
      ...
      groups_data: JSON.stringify(arrivalMembers.map((member: any) => ({
        type: 'member',
        id: member.id,
  ...
  for (const [departureKey, departureMembers] of departureGroups) {
    ...
    // Use original departure flight time
    const pickupTime = new Date(`${date.split('T')[0]}T${timeFormatted}`);
    console.log('🔍 DEBUG - Member departure transport - Original time:', time, 'Formatted:', timeFormatted);
    ...
    await createTransportSchedule({
      ...
      groups_data: JSON.stringify(departureMembers.map((member: any) => ({
        type: 'member',
        id: member.id,
        name: `${member.first_name} ${member.last_name}`,
        memberCount: 1
...
const createTransportSchedule = async (scheduleData: any) => {
  try {
    ...
    console.log('✅ Successfully created transport schedule with ID:', result.lastInsertRowid);
...
const updateGroupStatusIfNeeded = (group: any) => {
  if (group.tour_end_date && group.status !== 'completed' && group.status !== 'cancelled') {
    try {
      ...
      console.log(`Checking group ${group.id} (${group.group_name}): tour ends ${group.tour_end_date} (${tourEndDate.toISOString()}), current date: ${currentDate.toISOString()}, status: ${group.status}`);

      // If tour has ended, mark as completed
      if (tourEndDate < currentDate) {
        queries.getDatabase().prepare('UPDATE tour_groups SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run('completed', group.id);
        group.status = 'completed';
        console.log(`✅ Auto-updated group ${group.id} (${group.group_name}) status to completed - tour ended on ${group.tour_end_date}`);
      }
    } catch (error) {
      console.error(`Failed to auto-update status for group ${group.id}:`, error);
    }
  }
  return group;
};
...
const toSqliteValue = (value: any): string | null => {
  ...
  if (typeof converted !== 'string') {
    throw new Error(`Failed to convert value to string: ${typeof value} = ${value}`);
  }
  return converted;
};
...
const toSqliteNumber = (value: any): number => {
  ...
  if (isNaN(converted)) {
    throw new Error(`Failed to convert value to number: ${typeof value} = ${value}`);
  }
  return converted;
};
  ...
  try {
    ...
    const transaction = db.transaction(() => {
      ...
      for (let i = 0; i < members.length; i++) {
        ...
        console.log('Creating guest:', member.first_name, member.last_name, 'with email:', member.email);

        // Check if guest with exact same details already exists (to avoid duplicates)
        const existingGuest = queries.getDatabase().prepare(`
        ...
        while (true) {
          try {
            ...
            const emailCheck = queries.getDatabase().prepare('SELECT id FROM guests WHERE email = ?').get(emailToUse);
            if (!emailCheck) {
        ...
        try {
          ...
          throw new Error(`Failed to create guest ${member.first_name} ${member.last_name}: ${error.message}`);
        }
      }
      ...
      for (const guest of createdGuests) {
        queries.addGroupMember().run(toSqliteNumber(groupId), toSqliteNumber(guest.id), guest.is_leader ? 1 : 0);
      }
      ...
      if (members && members.length > 0) {
        ...
        for (const member of members) {
          if (member.id && existingMemberIds.includes(member.id)) {
            ...
            // Add to group
            queries.addGroupMember().run(toSqliteNumber(id), newMemberId, member.is_leader ? 1 : 0);
...
export const testAutoStatusUpdate: RequestHandler = (req, res) => {
  try {
    ...
    const testGroup = queries.getDatabase().prepare(`
...
export const manualTriggerAutoScheduling: RequestHandler = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to manually trigger auto-scheduling" });
  }
};
