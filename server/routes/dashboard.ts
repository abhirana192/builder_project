      ...
      if (arrival.groups_data) {
        try {
          ...
          const passengers = groups.flatMap((p: { id: number; type: string }) => {
            if (p.type === 'group') {
              const group = queries.getGroupById().get(p.id) as { group_name: string; arrival_flight_number?: string | null };
              ...
              if (!flight_number) {
                try {
                  const guest = queries.getGuestById().get(p.id) as { arrival_flight_number?: string | null };
                  if (guest && guest.arrival_flight_number) flight_number = guest.arrival_flight_number;
                } catch {}
              }
              const name = `${member?.first_name || ''} ${member?.last_name || ''}`.trim() || 'Unknown Member';
    ...
    const detailedDepartures = departures.map(departure => {
      if (departure.groups_data) {
        try {
          ...
          const passengers = groups.flatMap((p: { id: number; type: string }) => {
            if (p.type === 'group') {
              const group = queries.getGroupById().get(p.id) as { group_name: string; departure_flight_number?: string | null };
              ...
              if (!flight_number) {
                try {
                  const guest = queries.getGuestById().get(p.id) as { departure_flight_number?: string | null };
                  if (guest && guest.departure_flight_number) flight_number = guest.departure_flight_number;
                } catch {}
              }
              const name = `${member?.first_name || ''} ${member?.last_name || ''}`.trim() || 'Unknown Member';
              return [{ id: p.id, type: 'member', name, groupName: member?.group_name || 'No Group', groupId: member?.group_id ?? null }];
